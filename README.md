# Restaurant Ordering System (Python + MySQL)

A Python-based Restaurant Ordering System that allows customers to view a menu, place orders, and store order information in a MySQL database. The system simulates a real café ordering workflow, handling menu display, order selection, billing, and database storage.

This project demonstrates the integration of Python programming with relational databases, along with user interaction through a command-line interface.

# Features
- Interactive restaurant menu display
- Allows users to select food items and quantities
- Order confirmation system
- Automatic bill calculation
- Stores order data in a MySQL database
- Simple command-line interface
- Demonstrates database connectivity with Python
